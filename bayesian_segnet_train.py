...
with tf.name_scope('segnet_basic_model'):
...
def test(sess, summary_writer):
	...
	while True:
		try:
			...
			# evaluate the batch prediction accuracy
			cur_class_pred = np.argmax(cur_prob_pred, axis=-1).astype(np.int32)
			cur_batch_acc = (labels_val.astype(np.int32) == cur_class_pred).astype(np.float32)
		except tf.errors.OutOfRangeError:
			break
